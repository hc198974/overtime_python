            ...
            for id in self.ids:
                ...
                if id.value in self.night_num:
                    ...
                    if night_total > total:
                        ...
                        # 存入总加班时间
                        self.dictall[id.value] = {
                            k: round(v, 2) for k, v in truncated_dict.items()}
                        # 将加班时间写入统计表
                        for x in self.ws1.rows:
                            if x[5].value == self.month:
                                if x[1].value == id.value and x[1].value in self.night_num:
                ...
                if cell.value is not None:
                    ...
                    if cell.value not in self.night_num:
                        ...
                        ws3.cell(
                            ...
                            column=16,
                            # 夜班工作日按1倍计算
                            value=round(workday_fee * base_salary * 1, 2),
                        )
                    ...
                    if cell.value not in self.night_num:
                        ...
                        ws3.cell(
                            ...
                            column=17,
                            # 夜班公休日按1倍计算
                            value=round(weekday_fee * base_salary * 1, 2),
                        )
                    ...
                    if cell.value not in self.night_num:
                        ...
                        ws3.cell(
                            ...
                            column=18,
                            # 夜班节假日按2倍计算
                            value=round(holiday_fee * base_salary * 2, 2),
                        )
